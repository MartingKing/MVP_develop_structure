...
public class LightStatusBarCompat {
    ...
    /**
     * 判断是否是小米、魅族或者大于等于6.0系统
     */
    public static boolean isMeizuOrXiaomiOrM() {
        return (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP && MIUILightStatusBarImpl.isMe())
                || (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP && MeizuLightStatusBarImpl.isMe())
    ...
    /**
     * 判断小米、魅族、大于等于6.0系统，设置全屏
     */
    public static void fullScreen(Activity activity) {
        if (isMeizuOrXiaomiOrM()) {
     ...
     * 设置顶部状态栏占位控件高度
     */
    public static void setStatusViewHeight(Context context, View statusView, int bgColorId) {
        if (isMeizuOrXiaomiOrM() && statusView != null) {
            statusView.setBackgroundColor(context.getResources().getColor(bgColorId));
