...
public final class LogUtils {
    ...
    private static boolean sLogSwitch         = BuildConfig.DEBUG_MODE;  // log总开关，默认开
    private static boolean sLog2ConsoleSwitch = true;  // logcat是否打印，默认打印
    private static String  sGlobalTag         = "DHD";  // log标签
    ...
    public static class Config {
        private Config() {
            ...
            if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())
                    && ContextUtils.getContext().getExternalCacheDir() != null)
                sDefaultDir = ContextUtils.getContext().getExternalCacheDir() + FILE_SEP + "log" + FILE_SEP;
            else {
                sDefaultDir = ContextUtils.getContext().getCacheDir() + FILE_SEP + "log" + FILE_SEP;
            }
        }
